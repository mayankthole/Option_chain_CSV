...
from utils import (
    create_data_directories, setup_logging,
...
def save_option_chain_data(df, expiry_date, fetch_time, symbol):
    """Save option chain data to CSV file organized by expiry date, appending new data with blank lines"""
    try:
        # Get the data directory from config
        data_dir = ALL_SYMBOLS[symbol]['data_dir']
        
        # Create expiry date directory
        expiry_dir = os.path.join(data_dir, expiry_date.replace(' ', '_'))
        if not os.path.exists(expiry_dir):
            os.makedirs(expiry_dir)
        ...
        # Create filename with just the symbol and expiry
        filename = f"{symbol}_{expiry_date.replace(' ', '_')}.csv"
        ...
        # Add a blank line after the data
        with open(filepath, 'a') as f:
            f.write('\n')
            
        print(f"Successfully {'appended' if file_exists else 'created'} data to {filepath}")
...
def main():
    ...
    while True:
        try:
            ...
            if current_time.weekday() >= 7:  # 5 is Saturday, 6 is Sunday # correct this later
